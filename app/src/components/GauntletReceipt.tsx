...
export function GauntletReceipt({
  ...
  function handleChallenge() {
    const url = `${window.location.origin}?gauntlet=${gauntletId}`;
  ...
  return (
    ...
    >
      ...
      {run.rounds.map((round, i) => {
        ...
        const timeBonus = qualityGateMet ? (round.seconds_remaining ?? 0) * 10 : 0;
            ...
            {!skipped && (
              <div className="border-t border-zinc-100 dark:border-zinc-800 pt-3 space-y-1 text-xs font-mono text-gray-500 dark:text-zinc-500">
                ...
                {qualityGateMet ? (
                  <div className="flex justify-between">
                    <span>
                      Speed Bonus: {round.seconds_remaining}s × 10
                    </span>
                    <span>+{timeBonus}</span>
                  </div>
                ) : (
      ...
      >
        <Button onClick={handleChallenge} variant="secondary" className="flex-1">
          {copied ? (
            <>
